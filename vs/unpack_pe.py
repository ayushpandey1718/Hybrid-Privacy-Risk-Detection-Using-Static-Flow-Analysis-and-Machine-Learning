...
# 1. Generate a list of packed files from the malware sample set
#    using the packer ID feature file to select the relevant files.
# 2. Take the list of packed files and attempt to unpack them
#    and dump the decrypted/uncompressed memory blocks to a
#    a file.
#
# Input : packed_file_list.txt
#
# or
#
...
#                        [is_packed]
#
# Output: unpacked binary memory segments (uncompressed or decrypted memory blocks)
#         %s.memblk0x%x.dec
#         %s.memblk0x%x.decomp
# 
# or
#
# Output: list of packed binary files from the sample set.
...
#
# Included functions from unpack.py to do some modifications and make it
...
###
  ...
  def post_VirtualAllocEx(self,event,retval):
      try:
          ...
          log("[*] <{:d}:{:d}> 0x{:x}: VirtualAllocEx(0x{:x},0x{:x},0x{:x} ({:d}),0x{:x},0x%03x) = 0x{:x}".format((pid, tid, ra, hProcess, lpAddress, dwSize, dwSize, flAllocationType, flProtect, retval)))
...
###
  ...
  def single_step(self,exception):
    try:
      ...
      if (e_addr >= self.lowesteip) and (e_addr <= self.highesteip) and (not e_addr in self.disasmd):
        ...
        log("    0x{:x}: {:s}".format((e_addr,instr)))
  ...
  def exception(self, exception):
    log("[*] Unhandled exception at 0x{:x}: {:s}".format((exception.get_exception_address(), exception.get_exception_name())))
...
def unpack_pe_binaries(file_list):
    ...
    for file_name in file_list:
        ...
        if os.path.isfile(file_path):
            # call the debugger to attempt unpacking.
...
feature_file_name = options.featurefilename
out_packed_list_file_name = options.outputfilename
in_packed_list_file_name = options.inputfilename

logfile = open("unpack_pe.log", "w")
log("[*] Started at {:s}".format(time.strftime("%Y-%m-%d %H:%M:%S")))

if write_file_list == True:
    write_packed_file_list(feature_file_name, out_packed_list_file_name)
else:
    fip = open(in_packed_list_file_name)
    file_list = fip.readlines()
    unpack_pe_binaries(file_list)
    fip.close()
    
log("[*] Completed at {:s}".format(time.strftime("%Y-%m-%d %H:%M:%S")))
