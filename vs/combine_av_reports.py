...
if __name__ == "__main__":
    ...
    clamav_file = 'data/clamav-vs263-264-apt.csv'
    defender_file = 'data/defender-vs263-264-apt.csv'

    if not os.path.isfile(clamav_file):
        print("Warning: ClamAV report '{}' not found.".format(clamav_file))
    elif not os.path.isfile(defender_file):
        print("Warning: Defender report '{}' not found.".format(defender_file))
    else:
        clammals = pd.read_csv(clamav_file)
        windefmals = pd.read_csv(defender_file)
